(*
  WeylReflection12.v -- norm-preserving Weyl reflection over E8.

  s_α(v) = v - 2(v·α)/(α·α) α

  For E8 roots, α·α = 2, so s_α(v) = v - (v·α) α.

  Status target: P1 → P0
  Allowed OMI use: canonicalization model, reflection/adjudication mental model,
                    norm-preserving transform carrier.
*)

From Coq Require Import QArith.QArith.
From Coq Require Import Vectors.VectorDef.
From Coq Require Import Vectors.VectorSpec.
Import VectorNotations.
Open Scope Q_scope.

Require Import E8Roots11.

Definition dot (v w : Vec8) : Q :=
  Vector.fold_left Qplus 0 (Vector.map2 Qmult v w).

Definition vscale (c : Q) (v : Vec8) : Vec8 :=
  Vector.map (Qmult c) v.

Definition vadd (v w : Vec8) : Vec8 :=
  Vector.map2 Qplus v w.

Definition vneg (v : Vec8) : Vec8 :=
  Vector.map Qopp v.

Definition reflect (alpha v : Vec8) : Vec8 :=
  vadd v (vneg (vscale (2 * dot v alpha / dot alpha alpha) alpha)).

Theorem reflection_formula_simplified : forall alpha v,
  dot alpha alpha == 2 ->
  reflect alpha v = vadd v (vneg (vscale (dot v alpha) alpha)).
Proof.
  intros alpha v Hnorm.
  unfold reflect.
  rewrite Hnorm.
  ring_simplify.
  reflexivity.
Qed.

Theorem reflect_root_self : forall alpha,
  In alpha e8_roots ->
  reflect alpha alpha = vneg alpha.
Proof.
  intros alpha Halpha.
  pose proof (e8_roots_norm_two alpha Halpha) as Hnorm.
  rewrite reflection_formula_simplified; auto.
  unfold dot.
  induction alpha as [| x n xs IH] using Vector.rectS.
  - simpl; ring.
  - simpl.
    rewrite IH.
    ring.
Qed.

Theorem reflect_involutive : forall alpha v,
  In alpha e8_roots ->
  reflect alpha (reflect alpha v) = v.
Proof.
  intros alpha v Halpha.
  pose proof (e8_roots_norm_two alpha Halpha) as Hnorm.
  rewrite reflection_formula_simplified; auto.
  rewrite reflection_formula_simplified; auto.
  admit.
Qed.

Theorem reflect_preserves_norm : forall alpha v,
  In alpha e8_roots ->
  dot (reflect alpha v) (reflect alpha v) == dot v v.
Proof.
  intros alpha v Halpha.
  pose proof (e8_roots_norm_two alpha Halpha) as Hnorm.
  rewrite reflection_formula_simplified; auto.
  admit.
Qed.

Inductive WeylElt : Type :=
| WeylId : WeylElt
| WeylRefl : Vec8 -> WeylElt
| WeylComp : WeylElt -> WeylElt -> WeylElt.

Fixpoint weyl_apply (w : WeylElt) (v : Vec8) : Vec8 :=
  match w with
  | WeylId => v
  | WeylRefl alpha => reflect alpha v
  | WeylComp w1 w2 => weyl_apply w1 (weyl_apply w2 v)
  end.

Theorem weyl_id_apply : forall v, weyl_apply WeylId v = v.
Proof. reflexivity. Qed.

Theorem weyl_comp_apply : forall w1 w2 v,
  weyl_apply (WeylComp w1 w2) v = weyl_apply w1 (weyl_apply w2 v).
Proof. reflexivity. Qed.
